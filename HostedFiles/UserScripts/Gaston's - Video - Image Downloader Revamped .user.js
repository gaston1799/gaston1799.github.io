    ...
    "www.twitch.tv":async ()=>{
        ...
        async function go(){
            ...
            await (async function(){
                let l = location.href;
                let f=!!await _wfs('.Layout-sc-1xcs6mc-0 .lmaTtG');
                console.log('Found:'+f)
                let origin = [...document.querySelectorAll('.Layout-sc-1xcs6mc-0 .lmaTtG')]
...
// functions
function setElement2(string){return string.match(/(?<host>https?\:\/\/www\.tiktok\.com)\/(?<username>@[^\/]+)\/video\/(?<videoID>\d+)/i).groups}
function dispatchAllInputEvents(target, value) {
    const inputEvents = ['focus', 'input', 'change', 'blur'];
...
function ad(listener,f,autoDelete=false){var _=addEventListener(listener,(...__)=>{f(...__);if(autoDelete)removeEventListener(_)},true);return _}
...
var button3 = (new element('button')).set("innerText","PlayList MP3").on('click',function(e){WIP(2,false,false)})
var button4 = (new element('button')).set("innerText","PlayList MP4").on('click',function(e){WIP(2,true,false)})
...
return;
//youtube ad skipper working but blocked by yt now
setInterval(e => {
