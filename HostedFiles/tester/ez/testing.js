var _global=(typeof exports == "undefined" ? !this.Device ? (typeof window == "undefined" ? this : (window || globalThis || self || top)) : exports : this)
...
function old() {
    ...
    ; ((root) => {
        ...
        function encode(str = '', options = { strings: !!0, methods: !!0, line_breaks: !!0, numbers: !!0, regexs: !!0 }) {
            ...
            try {
                if (options.methods) {
                    ...
                    // method fixing
									//;(str.match(/[$_a-zA-Z\)\]]+\.[\.A-Z0-9a-z_$]+/gi) || []).forEach(f)
                    //str.match(/[^.][^0-9]\.\w+/g);
										str.match(/[$_a-zA-Z\)\]]+\.[\.A-Z0-9a-z_$]+/gi).map(e => [e, (e.replace(".", "['") + "']").split(".").join("']['")]).forEach(e => {
                    console.log('method edit',{e})
                    s = s.replace(e[0], e[1])
                    });
                }
            } catch (err) {
                ...
                if (options.strings) {
                    ...
                    cc.forEach(ee => {
                        s = s.replace(ee, arrid + "[" +
                            find(ee, [...c,...cc])[rand(find(ee, [...c,...cc]))]
                            + "]")
                    })
                }
            } catch (err) {
                console.log("No string fix needed seconds");
                console.log(err)
            }
            try {
            ...
            console.log({b,newstr,c,arrid})
            var fileSize = (lengthInUtf8Bytes('const ' + arrid + "=[" + ([...c,...cc].map(e => e.split("").map(Ecode).join("")).join(",")).split('\n').join('\\n') + "];" + b + newstr) / 1000).toFixed(2);
            var kko = ['const ' + arrid + "=[" + ([...c,...cc].map(e => e.split("").map(Ecode).join("")).join(",")).split('\n').join('\\n') + "];" + b + newstr, fileSize + 'KB'];
            var info = {
...
d.onchange=function(){
	...
	console.log(
		[...(_str.match(/\`.+`/gi)||[])].map(e=>{
			console.log(e)
			var found=false
			var [start,end,startLevel]=[0,0,0]
			var sl,el;
			var level=1
			var str=''
			var in_
			...
			e.split('').forEach((a,b)=>{
				...
				if(a=='$'&&next=='{'){
					str=''
					in_=true
					start=b;startLevel=level;
					//console.log([prev,a,next,level])
				}
				if(in_)str+=a;
				if(a=='}'&&prev!='\\'&&level==startLevel){
					end=b;
					in_=false
					//console.log([prev,a,next,level])
					//console.log(str)
			...
			var arr2=arr.map(e=>[e,e.split('').splice(2).reverse().splice(1).reverse().join("")])
			console.log(arr2)
			for(let b=1;b<e.length-1;b++){
				const a=e[b];
				var l=arr2.filter(c=>spells(e,b,c[0]))[0]
				if(a=="'")newString+="\\'"
				else if(l){
					b+=l[0].length-1
					newString+=`'+${l[1]}+'`
			...
			newString+='\''
			_str=_str.replace(e,newString)
		})
	)

	function spells(str,start,str2){
		return str.split("").splice(start,str2.length).join("")==str2
	}
	//console.log(_str.match(/\/\*{2}(.+|\s)?(( |\t|\s)+(\*|\s)+(^\/)?(\@|\s)+.+){1,}\*\//gi))
    d.value= _str
	var gsClass=Encoded.makeid(2)
	var ex=new Encoded(_str)
	var res=ex.result
	var globals=res.match(/[\w_\$]+\[/gi)
	var gs=[]
	var gsVar=Encoded.makeid(2)
	var gsClass=Encoded.makeid(2)
	/*
if(globals){
	[...globals].forEach(e=>{
		let name=e.match(/[\w_\$]+/)[0]
		console.log(name)
		name&&_global[name]&&(gs.push(name),gs=[...new Set(gs)],res=res.split_replace(`${name}[`,null,`${gsVar}._${name}[`)
		)
	})
}
...
res=`const ${gsVar}={
	${gs.map(name=>
		`get _${name}(){return this.${name}=${name}},set _${name}(a){return this.${name}=this.${name}}`
	).join(',\n')}
}
${res}`*/
    try{const _t=['\u0066\u0069\u006c\u0074\u0065\u0072','\u006c\u0065\u006e\u0067\u0074\u0068','\u006d\u0061\u0074\u0063\u0068','\u0066\u006f\u0072\u0045\u0061\u0063\u0068','\u006a\u006f\u0069\u006e','\u0072\u0065\u0070\u006c\u0061\u0063\u0065','\u006d\u0061\u0070','\u0073\u0070\u006c\u0069\u0074','\u006d\u0061\u0070','\u0073\u0070\u006c\u0069\u0074','\u006a\u006f\u0069\u006e'];;[...res[_t[0x0002]](/\/\*{2}([^\/])+./gi)][_t[0x0006]](e=>{var f=e[_t[0x0009]]('*')[_t[0x0004]]('\n*')[_t[0x0009]]('\n')[_t[0x0000]](e=>e[_t[0x0001]]>0x0002);return [e,'/**\n'+f[_t[0x0006]](g=>" "+g)[_t[0x000A]]('\n')+'\n */']})[_t[0x0003]](([a,c])=>{res=res[_t[0x0005]](a,'\n'+c)})}catch(err){}
	document.getElementById("b").value=res.split('*/;').join('*/')
}